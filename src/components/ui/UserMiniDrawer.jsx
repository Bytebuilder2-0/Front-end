    ...
    const fetchAppointments = async () => {
      try {
        ...
        const filtered = appointmentsData.filter(
          (appt) =>
            appt &&
            !["Cancelled", "All done", "Reject1", "Paid"].includes(appt.status)
        );
  ...
  const menuItems = useMemo(
    () => [
      ...
      },
      {
        path: "/Vehicles",
        label: "Your Vehicles",
        icon: <DirectionsCarIcon sx={{ color: "#ffffff" }} />,
      },
  ...
  return (
    <Box sx={{ display: "flex" }}>
      ...
      <Drawer
        ...
        open={open}
        sx={{
          "& .MuiDrawer-paper": { backgroundColor: "#33383E", color: "white" },
        }}
      >
        <DrawerHeader />
        <Divider />
        ...
        {open && (
          ...
          >
            ...
            >
              {!profile?.profilePhoto && profile?.name
