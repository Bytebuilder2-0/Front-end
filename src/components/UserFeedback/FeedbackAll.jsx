...
const FeedbackAll = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      console.log(
        "Appointments with feedbackStatus: true",
        appointmentsData.filter((appt) => appt.feedbackStatus)
      );
        ...
        if (appointment.feedbackStatus && appointment._id) {
          try {
            const res = await axios.get(
              `${API_URL}/feedback/${appointment._id}`,
              {
                headers: { Authorization: `Bearer ${token}` },
              }
            );
            ...
            console.error(
              `Failed to fetch feedback for appointment ${appointment._id}`,
  ...
  const pendingFeedbacks = appointmentsWithFeedback.filter(
    (appt) =>
      (appt.status === "Task Done" ||
        appt.status === "Paid" ||
        appt.status === "All done") &&
      appt.feedbackStatus === false
  );
  ...
  return (
    <Box sx={{ width: "100%", padding: 3 }}>
      ...
      <Typography color="text.secondary" sx={{ mb: 3 }}>
        Track your pending feedback and review your feedback history in one
        place.
      </Typography>
      ...
      {tabValue === 1 && (
        <FeedbackHistory feedbacks={completedFeedbacks} loading={loading} />
      )}
